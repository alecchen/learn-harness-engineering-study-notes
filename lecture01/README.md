...
- Run the suite after the agent finishes each task, enforced by a pre-commit hook or a local CI runner ([gitlab-ci-local](https://github.com/firecow/gitlab-ci-local) for GitLab, [act](https://github.com/nektos/act) for GitHub), not just a prompt rule.
...
- **Local gate** - pre-commit hook or a local CI runner ([gitlab-ci-local](https://github.com/firecow/gitlab-ci-local) for GitLab, [act](https://github.com/nektos/act) for GitHub) runs the suite per task. Fast, but `-n`-able, so it is the per-task loop, not the floor.
